...
@Component({
  selector: 'app-dit-nhat',
  templateUrl: './dit-nhat.component.html',
  styleUrls: ['./dit-nhat.component.css']
})
export class DitNhatComponent implements OnInit {
  numbers: Numbers[] = [];
  ...
  splitInputData() {
    ...
    let isValidType = contents[0] == 'Dit nhat' || contents[0] == 'Đít nhất' || contents[0] == 'đít nhất' || contents[0] == 'dit nhất' || contents[0] == 'dit nhat' || 'nhat' || 'nhất';
    if (isValid && isValidType) {
      ...
      this.message = 'Xin hãy nhập như mẫu sau: Đít nhất: 01,02x1n';
      $('#modal-danger').modal('show');
    }
  }
  ...
  submit() {
    ...
    this.gamePlayService.play(data).subscribe(() => {
      ...
      if (localStorageArray == null) {
        localStorage.setItem('dit-nhat', JSON.stringify(this.resultNumbers));
      } else {
        localStorageArray = JSON.parse(localStorage.getItem('dit-nhat'));
        this.resultNumbers.map(number => {
          ...
          localStorage.setItem('dit-nhat', JSON.stringify(localStorageArray));
        });
      }
      this.clearAll();
    }, () => {
